...
IFIDX=$(docker exec kind-control-plane \
...
docker ps
LB_IP=$(docker exec kind-control-plane ip -o -4 a s eth0 | awk '{print $4}' | cut -d/ -f1)

docker exec kind-worker2 /bin/sh -c 'apt-get update && apt-get install -y nginx && systemctl start nginx'
WORKER2_IP=$(docker exec kind-worker2 ip -o -4 a s eth0 | awk '{print $4}' | cut -d/ -f1)
